...
import { getNowTime, getArrayIndex, removeDuplicate } from "../../utils/utils";
...
export default function Fruit() {
  ...
  function search(e) {
    if (e.detail) {
      ...
      var newSearchArray1 = List.filter((item) => {
        return item.kind.includes(content);
      });
      var totalSearchArray = [];
      for (let i = 0; i < newSearchArray.length; i++) {
        const e = newSearchArray[i];
        totalSearchArray.push(e);
      }
      for (let i = 0; i < newSearchArray1.length; i++) {
        const e = newSearchArray1[i];
        totalSearchArray.push(e);
      }

      var result = removeDuplicate(totalSearchArray);
      console.log(result);
      setSearchArray(result);
      if (newSearchArray.length === 0 && newSearchArray1.length === 0) {
        Taro.showToast({
          title: "未找到水果",
          icon: "error",
          duration: 2000,
      ...
      <view className="search">
        ...
        >
          {searchArray.map((item) => {
            return (
              <view className="all" key={item.id}>
                <view className="left">
                  <view className="Number">
                    <Image src={A} className="arrow"></Image>
                  </view>
                  <view>
                    <Image src={item.src} className="picture"></Image>
                  </view>
                </view>
